"""
Parse Asprova-style Output Instruction (PSI) CSV exports (e.g. outputinst.csv).

Column detection follows the same approach as input_instruction_parser.py.
"""
...
def detect_output_instruction_columns(headers: list[str]) -> Dict[str, str]:
    ...
    mapping["item_code"] = find_col("outputworkinst_itemcode", "itemcode", "item_code")
    mapping["inst_time"] = find_col("outputworkinst_time", "inst_time", "time")
    mapping["quantity"] = find_col("outputworkinst_qty", "qty", "quantity")
    mapping["u_quantity"] = find_col("outputworkinst_uqty", "uqty", "u_quantity")
    mapping["qty_fixed_level"] = find_col("outputworkinst_qtyfixedlevel", "qtyfixedlevel")
    mapping["qty_fixed_level_user_specified"] = find_col(
        "outputworkinst_qtyfixedlevel_userspecified",
        "qtyfixedlevel_userspecified",
        "qtyfixedlevel_user",
    )
    mapping["pegging_method"] = find_col("outputworkinst_peggingmethod", "peggingmethod")
...
def parse_output_instruction_row(
    row: Mapping[str, Any], mapping: Mapping[str, str]
) -> Optional[Dict[str, Any]]:
    """
    Parse one CSV row into values for psi_output_instructions.
